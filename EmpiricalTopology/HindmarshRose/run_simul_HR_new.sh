#!/bin/bash
# Simultaneous inference (structure + dynamics) on degree-matched HR Celegans subgraphs
# using COMPONENT-R² validation (self_x/y/z + Aij*G vs soft_w*msg).
#
# Output: HR_dyn/results_simul_new/

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
TRAIN_SCRIPT="${SCRIPT_DIR}/train_simul_HR_new.py"

BASE_OUTDIR="${SCRIPT_DIR}/results_simul_new"
...
max_per_gpu_for_nodesize() {
    local nodesize="$1"
    if [ "$nodesize" -le 200 ]; then
        echo 6
    else
        echo 3
    fi
...
wait_for_slot() {
    local nodesize="$1"
    local max_per_gpu
    max_per_gpu=$(max_per_gpu_for_nodesize "$nodesize")
    while true; do
        for g in "${GPUS[@]}"; do
            local cnt=0
...
for spec in "${NODE_SPECS[@]}"; do
    ...
    for n_train in "${TRAIN_SAMPLES[@]}"; do
        for seed in "${SEEDS[@]}"; do
            ...
            GPU_ID=$(wait_for_slot "${NUM_NODES_KEEP}")
            TOTAL=$((TOTAL + 1))
...
echo ""
echo "All ${TOTAL} component-R² simul jobs launched (${SKIPPED} skipped). Waiting..."
wait
echo "All component-R² simul experiments completed."
