...
SERIES_RE = re.compile(
    r"Series_N(?P<N>\d+)_Lorenz_T(?P<T>\d+)_dt(?P<dt>[^_]+)_epsilon(?P<epsilon>[^_]+)_d(?P<d>[^_]+)_seed(?P<seed>\d+)\.pickle$"
)
...
def parse_series_filename(path):
    ...
    return {
        ...
        "T": int(meta["T"]),
        "dt_tag": meta["dt"],
        "epsilon_tag": meta["epsilon"],
        "d_tag": meta["d"],
        "seed": int(meta["seed"]),
        "epsilon": tag_to_float(meta["epsilon"]),
