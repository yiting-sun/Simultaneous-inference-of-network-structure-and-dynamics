...
edge_index_i2j = get_edge_index(initialA)
index = [1,0]
edge_index_j2i = edge_index_i2j[index]
...
Dyn = Get_sign_MP(n_f, msg_dim, dim, dt, edge_num, tau=tau, lam=lam, aggr=aggr, hidden=hidden).to(device)
...
weights_save_path = f"{save_path}weights_tau{tau_updata}_lam{lam}_{timestamp}.pkl"
