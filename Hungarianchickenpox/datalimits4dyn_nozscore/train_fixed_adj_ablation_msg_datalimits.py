"""
Training Script: Fixed Adjacency + Message-Feature Ablation + Data-Limits Sweep
================================================================================
Same as ``train_fixed_adj_ablation_msg.py`` but with an additional knob:
``--num_train_samples`` uniformly downsamples the *training* set to study how
much data is needed to learn the dynamics (cf. ``reference/knstru_batch.py``).

IMPORTANT — feature construction order:
    Features (rolling means, lagged differences, seasonal components, ...) are
    ALL built on the FULL, uncut time series first. Only AFTER the feature
    tensor is built do we uniformly subsample along the time axis. This way
    the feature values for any retained timestamp are identical to what they
    would have been on the full dataset — the reduction only changes which
    timestamps the model trains on, not what each timestamp "sees".

Validation and test sets are kept full so different ``--num_train_samples``
runs are comparable on the same evaluation set.
"""
...
def uniform_subsample_indices(length, num_samples):
    ...
    Mirrors ``reference/tools.py::get_index`` semantics: spread points as
    evenly as possible across the full range, including both endpoints when
    num_samples >= 2. The returned indices are unique and sorted.
    """
    if num_samples <= 0:
    ...
    # In the unlikely case rounding collapsed two points, top up with nearest
    # unused indices to keep the count exact.
...
def main():
    parser = argparse.ArgumentParser(description="Fixed adj + msg ablation training with training-data subsampling")
    ...
    parser.add_argument("--batch_ratio", type=float, default=0.1,
                        help="Batch size as a fraction of the (subsampled) training set, "
                             "so the number of optimizer steps per epoch is roughly constant "
                             "across different --num_train_samples. Set <= 0 to fall back to --batch.")
    ...
    print(f"{'=' * 60}")
    print(f"Fixed Adj Ablation (data-limits): adj_type={args.adj_type}, msg={msg_config}")
    print(f"zscore={args.zscore}, smooth={args.smooth}, seed={args.seed}")
    print(f"num_train_samples={args.num_train_samples}")
    ...
    # NOTE: features are built on the FULL time series — they MUST be computed
    # before any subsampling so that rolling means / lagged diffs / seasonal
    # components see the full history. Subsampling only chooses which
    # already-built feature rows the model trains on.
    ...
    X_train_full, Y_train_full = X_t[:train_end], Y_t[:train_end]
    ...
    for epoch in tqdm(range(args.epochs), desc=f"{args.adj_type}_{msg_config}_n{effective_num_train}"):
        model.train()
    ...
    print(f"\n{'=' * 60}")
    print(f"FINAL RESULTS ({args.adj_type}, {msg_config}, n_train={effective_num_train}):")
