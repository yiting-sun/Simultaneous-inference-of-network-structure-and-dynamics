#!/bin/bash
# Run all fixed-adj message-feature ablation experiments
# Distributes jobs across 4 GPUs (cuda:0-3), max 5 concurrent per GPU.

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
TRAIN_SCRIPT="${SCRIPT_DIR}/train_fixed_adj_ablation_msg.py"
BASE_OUTDIR="${SCRIPT_DIR}/runs_infer_adj_msg"
    ...
    for msg in "${MSG_CONFIGS[@]}"; do
        for zscore in "${ZSCORES[@]}"; do
            ...
            for seed in "${SEEDS[@]}"; do
                RUN_NAME="run${seed}_${adj_type}_${msg}_${zscore_tag}_gaussian"
...
wait
echo "All fixed-adj message-feature ablation experiments completed."
