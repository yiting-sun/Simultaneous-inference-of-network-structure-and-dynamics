"""
Training Script: Fixed Adjacency + Msg-Feature Ablation + Data-Limits + Sub-Nodes
=================================================================================
Same as ``train_fixed_adj_ablation_msg_datalimits.py`` but with one extra knob:
``--num_nodes_keep`` shrinks the 20-county network to a smaller graph by
*dropping* nodes. The set of dropped nodes is selected reproducibly from the
``--seed`` value, so different seeds drop different nodes while a given seed
always produces the same drop set. The adjacency loaded from the saved npz
files (originally 20x20) is reduced to (num_nodes_keep, num_nodes_keep) by
removing the same rows/columns.

Order of operations (matters!):
    1. Drop nodes from the raw time series (and county_names).
    2. Build features on the FULL (in time) but reduced-node series.
    3. 60/20/20 temporal split.
    4. Uniform subsample of training rows.
    5. Drop the matching rows/cols from the loaded adjacency.

This way the dropped nodes are completely absent from features, targets, model
inputs, and the message graph.
"""
...
def select_kept_node_indices(total_nodes, num_keep, seed):
    ...
    Uses a dedicated ``np.random.default_rng(seed)`` so the choice is fully
    determined by ``seed`` and independent of the global numpy/torch RNG state
    (so model init reproducibility is unaffected). Returns a sorted np.int64
    array of length ``num_keep``.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Fixed adj + msg ablation training with training-data subsampling AND node subsampling")
    ...
    # ---- node-limits knob ---------------------------------------------------
    parser.add_argument("--num_nodes_keep", type=int, default=15,
                        help=f"Number of nodes to KEEP out of the full {ORIGINAL_NUM_NODES}-node "
                             f"graph. The {ORIGINAL_NUM_NODES - 15} dropped nodes are selected "
                             "reproducibly from --seed (different seeds drop different nodes).")
    ...
    print(f"{'=' * 60}")
    print(f"Fixed Adj Ablation (data-limits + sub-nodes): "
          f"adj_type={args.adj_type}, msg={msg_config}")
    print(f"zscore={args.zscore}, smooth={args.smooth}, seed={args.seed}")
    print(f"num_train_samples={args.num_train_samples}, num_nodes_keep={args.num_nodes_keep}")
    ...
    # Smoothing — applied AFTER node selection (per-column smoothing is
    # independent across nodes, so dropping nodes first is mathematically the
    # same as smoothing then dropping).
    X = gaussian_filter1d(X_raw, sigma=1.5, axis=0, mode="nearest") if args.smooth == "gaussian" else X_raw

    # Features built on the FULL time series (in time) of the kept nodes.
    ...
    desc = f"{args.adj_type}_{msg_config}_nN{num_nodes}_n{effective_num_train}"
