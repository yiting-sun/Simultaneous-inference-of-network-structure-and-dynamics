"""
Training Script: Simultaneous Inference + Msg-Feature Ablation
              + Data-Limits Sweep + Sub-Nodes Sweep
==============================================================
Same as ``train_ablation_msg.py`` but with two extra knobs (mirroring
``datalimits4dyn_nozscore/train_fixed_adj_ablation_msg_subnodes.py``):

* ``--num_nodes_keep`` shrinks the 20-county graph by *dropping* nodes. The
  set of dropped nodes is selected reproducibly from ``--seed`` via a
  dedicated ``np.random.default_rng(seed)`` so different seeds drop
  different nodes while a given seed always produces the same drop set.
  The ground-truth adjacency loaded from ``--adj_npy`` is reduced from
  20x20 to ``num_nodes_keep × num_nodes_keep`` by removing the same rows
  and columns.
* ``--num_train_samples`` uniformly subsamples the *training* set after
  features are built on the FULL (in time) series — so feature values for
  the retained timestamps are identical to what they would have been on
  the full dataset; the reduction only changes which timestamps the model
  sees during training. Validation and test are kept at full size for
  fair comparison.

Order of operations:
    1. Drop columns from raw time series (and county_names).
    2. (optional) log1p, then smoothing — both per-column, so commute with
       column dropping.
    3. Build features on the full reduced-node series.
    4. 60/20/20 temporal split.
    5. Uniformly subsample training rows.
    6. Slice the loaded ground-truth adjacency to the kept nodes.

A ``--batch_ratio`` (default 0.1) replaces the fixed batch size so that
the number of optimizer steps per epoch stays roughly constant across
different ``--num_train_samples``.
"""
...
ORIGINAL_NUM_NODES = 20  # chickenpox Hungary = 20 counties
...
def uniform_subsample_indices(length: int, num_samples: int) -> np.ndarray:
...
def select_kept_node_indices(total_nodes: int, num_keep: int, seed: int) -> np.ndarray:
    """Reproducibly choose ``num_keep`` node indices to RETAIN out of ``total_nodes``.

    Uses an independent ``np.random.default_rng(seed)`` so the choice does not
    perturb the global numpy/torch RNG state used for model initialisation.
    """
...
def compute_structure_metrics_soft(true_adj: np.ndarray, pred_adj: np.ndarray) -> dict:
    ...
    # Guard against the rare degenerate case where the kept-node sub-adjacency
    # has all-zero or all-one labels (AUC/AUPRC undefined).
...
def main():
    parser = argparse.ArgumentParser(
        description="Simultaneous-inference msg-feature ablation training "
                    "with training-data subsampling AND node subsampling")
    ...
    parser.add_argument("--batch_ratio", type=float, default=0.1,
                        help="Batch size as a fraction of the (subsampled) training set, so the "
                             "number of optimizer steps per epoch stays roughly constant across "
                             "different --num_train_samples. Set <= 0 to fall back to --batch.")
    ...
    parser.add_argument("--num_train_samples", type=int, default=-1,
                        help="Uniformly subsample the training set to this many samples "
                             "AFTER features are built on the full series. -1 = use all.")
    # ---- node-limits knob ---------------------------------------------------
    ...
    print(
        f"Node dropping: kept {len(kept_node_indices)}/{full_n} nodes "
    ...
    # --- Optional log transform + smoothing (per-column, OK after drop) --
    X = np.log1p(X_raw) if args.log else X_raw
    X = maybe_smooth(X, args.smooth)

    # --- Build features on the FULL (in time) reduced-node series --------
    X_feat, dXdt, feature_names = build_longwindow_features_t(
        X=X, dt=args.dt, seasonal_period=args.seasonal_period
    )
    X_t = torch.tensor(X_feat, dtype=torch.float32)
    Y_t = torch.tensor(dXdt, dtype=torch.float32)

    # --- Slice the ground-truth adjacency to kept nodes ------------------
    A_true_full = np.load(args.adj_npy)
    if A_true_full.ndim != 2 or A_true_full.shape[0] != A_true_full.shape[1]:
        raise ValueError(f"Adjacency must be square, got {A_true_full.shape}")
    if A_true_full.shape[0] != full_n:
        raise ValueError(
            f"Adjacency size {A_true_full.shape[0]} != full node count {full_n}"
        )
    A_true_full = (A_true_full != 0).astype(np.float32)
    np.fill_diagonal(A_true_full, 0.0)
    A_true = A_true_full[np.ix_(kept_node_indices, kept_node_indices)].astype(np.float32)
    ...
    # --- Uniform subsampling on the (already-built) training features ----
    ...
    val_loader = DataLoader(val_ds, batch_size=max(1, effective_batch // 2), shuffle=False, num_workers=0)
    test_ds = WeightedGraphDataset(X_test, y_test, edge_index)
    test_loader = DataLoader(test_ds, batch_size=max(1, effective_batch // 2), shuffle=False, num_workers=0)
    ...
    np.save(os.path.join(args.outdir, "ground_truth_adjacency_full.npy"), A_true_full)
    ...
    for epoch in tqdm(range(args.epochs), desc=f"{msg_config}_N{num_nodes}_n{effective_num_train}"):
        # --- Train ---
        Dyn.train()
        tr_loss = 0.0
        tr_items = 0
        ...
        if (epoch + 1) % 50 == 0 or (epoch + 1 == args.epochs):
            ...
            print(
                ...
                f"Adj_AUC={metrics_row['struct_auc']} "
                f"Adj_AUPRC={metrics_row['struct_auprc']} "
                f"Adj_corr={metrics_row['struct_corr']} "
    ...
    # --- Compact results.json (best-model metrics, always written) ---
    # NOTE: at this point Dyn already has best_state loaded back in (see above),
    # so the "final_*_metrics" computed above ARE the best-model metrics.
    # We dump them to results.json so downstream notebooks can read a single
    # small file regardless of whether `metrics_every` ever fired.
    def _f(x):
        try:
            v = float(x)
    ...
    results_summary = {
        ...
        "early_stopped": bool(epochs_without_improvement >= args.early_stop_patience),
        "best_val_mse": _f(best_val),
        "train": {
            "mse": _f(train_hist[-1] if len(train_hist) else np.nan),
            "r2": _f(final_train_metrics["overall_r2"]),
            "node_r2_mean": _f(final_train_metrics["node_r2_mean"]),
        },
        "val": {
            "mse": _f(val_hist[-1] if len(val_hist) else np.nan),
            "r2": _f(final_val_metrics["overall_r2"]),
            "node_r2_mean": _f(final_val_metrics["node_r2_mean"]),
        },
        "test": {
            "mse": _f(test_hist[-1] if len(test_hist) else np.nan),
            "r2": _f(final_test_metrics["overall_r2"]),
            "node_r2_mean": _f(final_test_metrics["node_r2_mean"]),
        },
        "structure": {
            "auc": _f(final_structure_metrics.get("auc")),
            "auprc": _f(final_structure_metrics.get("auprc")),
            "corr": _f(final_structure_metrics.get("corr")),
            "mse": _f(final_structure_metrics.get("mse")),
            "mae": _f(final_structure_metrics.get("mae")),
        },
        "config": {
            "lam": _f(getattr(args, "lam", None)),
            "tau": _f(getattr(args, "tau", None)),
    ...
    # --- Summary ---
    print(f"\n=== Final Results (msg_config={msg_config}, N={num_nodes}, n_train={effective_num_train}) ===")
    ...
    print(f"Adj AUC:  {final_structure_metrics['auc']}")
    print(f"Adj AUPRC: {final_structure_metrics['auprc']}")
    print(f"Adj Corr: {final_structure_metrics['corr']}")
